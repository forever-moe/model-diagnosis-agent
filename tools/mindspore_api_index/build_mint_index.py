...
def build_methodology() -> str:
    lines = [
        "# MindSpore API Analysis Methodology",
        "",
...
def main() -> None:
    ...
    parser.add_argument("--repo", required=True, help="Path to the MindSpore repo root.")
    ...
    main_payload = {
        "meta": {
            ...
            "repo_commit_hint": repo_commit_hint(repo_root),
            "mindspore_version_hint": mindspore_version_hint(repo_root),
    ...
    evidence_payload = {
        "meta": {
            "repo_commit_hint": main_payload["meta"]["repo_commit_hint"],
            "mindspore_version_hint": main_payload["meta"]["mindspore_version_hint"],
            "generated_at": main_payload["meta"]["generated_at"],
            "index_schema_version": main_payload["meta"]["index_schema_version"],
    ...
    yaml_dump(canonicalize_for_yaml(queue), out_dir / "mint_api_review_queue.yaml")
    (out_dir / "mint_api_index_review.md").write_text(build_review_markdown(main_records, queue), encoding="utf-8")
    (out_dir / "mint_api_index_rulebook.md").write_text(build_rulebook(), encoding="utf-8")
    (out_dir / "mindspore_api_analysis_methodology.md").write_text(build_methodology(), encoding="utf-8")
    remove_legacy_outputs(out_dir)

    summary = {
        "apis": len(main_records),
        "files": [
            str(out_dir / "mint_api_index.yaml"),
            str(out_dir / "mint_api_evidence.yaml"),
